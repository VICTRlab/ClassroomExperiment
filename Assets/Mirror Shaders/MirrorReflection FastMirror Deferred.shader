...
/////// Diffuse:
                ...
                float3 diffuseColor = (_ReflectionTex_var.rgb*(_ReflectionTex_var.a+saturate(((1.0 - _ReflectionTex_var.a)*((texCUBElod(__Cubemap,float4(viewReflectDirection,lerp(0,6,_BlurReflection))).rgb*lerp(0.0,3.0,_CubemapIntensity))+(node_7200*node_7200*(node_7200*1.0+0.0))))))*_ColorTint.rgb);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _ReflectionTex_var = tex2D(_ReflectionTex,TRANSFORM_TEX(sceneUVs.rg, _ReflectionTex));
                float node_7200 = pow(1.0-max(0,dot(normalDirection, viewDirection)),lerp(10.0,1.0,_CubemapEdges));
                float3 diffColor = (_ReflectionTex_var.rgb*(_ReflectionTex_var.a+saturate(((1.0 - _ReflectionTex_var.a)*((texCUBElod(__Cubemap,float4(viewReflectDirection,lerp(0,6,_BlurReflection))).rgb*lerp(0.0,3.0,_CubemapIntensity))+(node_7200*node_7200*(node_7200*1.0+0.0))))))*_ColorTint.rgb);
                o.Albedo = diffColor;
